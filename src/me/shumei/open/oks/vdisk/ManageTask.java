...
public class ManageTask {
	...
	public View initView(Context context, View view, String cfg) {
		...
		try {
			CheckBox cb_sendweibo = (CheckBox) view.findViewById(R.id.cb_sendweibo);
			JSONObject jsonObj = new JSONObject(cfg);
			boolean sendweibo = jsonObj.getBoolean("sendweibo");//true=>发送，false=>不发送
			cb_sendweibo.setChecked(sendweibo);
	...
	public boolean dataValidator(Context context, View view) {
		boolean flag = true;//不需要对自定义配置数据只有一个CheckBox，不需要作检测
		return flag;
	}
	...
	public String getConfigData(Context context, View view) {
		...
		try {
			CheckBox cb_sendweibo = (CheckBox) view.findViewById(R.id.cb_sendweibo);
			jsonObj.put("sendweibo", cb_sendweibo.isChecked());
		} catch (JSONException e) {
			e.printStackTrace();
		}
